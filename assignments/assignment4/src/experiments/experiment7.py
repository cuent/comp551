...
class MNIST_vae():
    ...
    def train(self, epoch):
        ...
        for batch, (x, c) in enumerate(self.train_loader):
            c = idx2onehot(c, n=10)
            x, c = x.to(self.device), c.to(self.device)
            y, m, logvar = self.model(x, c)
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            for batch, (x, c) in enumerate(self.test_loader):
                c = idx2onehot(c, n=10)
                x, c = x.to(self.device), c.to(self.device)
                y, mu, logvar = self.model(x, c)
    ...
    def execute(self, num_epochs):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            with torch.no_grad():
                sample = torch.randn(64, self.n).to(self.device)  # sampling 64 images
                sample = self.model.decoder(sample, cc)
